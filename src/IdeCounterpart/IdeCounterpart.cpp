...
namespace iqrf {
  ...
  void IdeCounterpart::attachInterface(iqrf::IAuthService* iface) {
    m_authService = iface;
  }

  void IdeCounterpart::detachInterface(iqrf::IAuthService* iface) {
  ...
  void IdeCounterpart::attachInterface(iqrf::IMessagingSplitterService* iface) {
    ...
    m_splitterService->registerServiceModeCheckHandler([this]() -> bool {
  ...
  void IdeCounterpart::detachInterface(iqrf::IMessagingSplitterService* iface) {
    if (m_splitterService == iface) {
      m_splitterService->unregisterServiceModeCheckHandler();
  ...
  void IdeCounterpart::setMode(Mode mode) {
    ...
    m_mode = mode;
    executeModeChangeCallbacks();

    TRC_INFORMATION("Set mode " << ModeStringConvertor::enum2str(m_mode));
    TRC_FUNCTION_LEAVE("");
  }

  void IdeCounterpart::registerModeChangeCallback(const std::string &instanceId, std::function<void()> callback) {
    std::lock_guard<std::mutex> lck(m_callbackMutex);
    m_setModeCallbacks.insert_or_assign(instanceId, callback);
  }

  void IdeCounterpart::unregisterModeChangeCallback(const std::string &instanceId) {
    std::lock_guard<std::mutex> lck(m_callbackMutex);
    m_setModeCallbacks.erase(instanceId);
  }

  void IdeCounterpart::processClientDisconnected(const MessagingInstance& messaging) {
    ...
    if (messaging.type != MessagingType::WS || !messaging.session.has_value()) {
      return;
    }
    // check if disconnected session manages service mode
    auto owner = m_serviceModeOwner.value();
    auto session = owner.session.value();
    auto candidate = messaging.session.value();
  ...
  void IdeCounterpart::executeModeChangeCallbacks() {
    std::lock_guard<std::mutex> lck(m_callbackMutex);
  ...
  IdeCounterpart::StatusCode IdeCounterpart::gatewayIdentification(rapidjson::Document& response, const MessagingInstance& messaging) {
    ...
    auto sessionInfo = messaging.session.value();
    if (!m_authService->hasServiceModePermissions(sessionInfo.getTokenId())) {
  ...
  IdeCounterpart::StatusCode IdeCounterpart::deactivateServiceMode(const MessagingInstance& messaging) {
    ...
    auto sessionInfo = messaging.session.value();
    auto token = m_authService->getApiToken(sessionInfo.getTokenId());
    if (!m_authService->hasServiceModePermissions(sessionInfo.getTokenId())) {
      return StatusCode::InsufficientPermission;
    }
    std::lock_guard<std::mutex> lock(m_modeMtx);
