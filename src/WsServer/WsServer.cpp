...
namespace iqrf {
  ...
  void WsServer::activate(const shape::Properties *props) {
    ...
    if (m_certPath.has_value() && std::filesystem::exists(m_certPath.value())) {
      m_ctx->use_certificate_chain_file(*m_certPath);
    }
    if (m_keyPath.has_value() && std::filesystem::exists(m_keyPath.value())) {
  ...
  void WsServer::modify(const shape::Properties *props) {
    ...
    m_address = boost::asio::ip::make_address(std::string("127.0.0.1"));
