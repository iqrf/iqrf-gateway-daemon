...
namespace iqrf {
  ...
  class JsDriverGenericSolver //: public DpaCommandSolver
  {
  protected:
    std::string m_driverRetval;
    IJsRenderService* m_iJsRenderService = nullptr;

  public:
    virtual ~JsDriverGenericSolver() {}

    JsDriverGenericSolver(IJsRenderService* iJsRenderService, uint16_t nadr = -1)
      //:DpaCommandSolver(nadr)
      : m_iJsRenderService(iJsRenderService)
    {}

    JsDriverGenericSolver(IJsRenderService* iJsRenderService, uint16_t nadr, uint16_t hwpid)
      //:DpaCommandSolver(nadr, hwpid)
      : m_iJsRenderService(iJsRenderService)
    {}

    std::string getDriverRequest()
    {
      TRC_FUNCTION_ENTER("");
      ...
      // call request driver func, it returns request result in text form
      try {
