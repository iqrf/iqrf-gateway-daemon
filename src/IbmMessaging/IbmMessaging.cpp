...
#include "iqrf__IbmMessaging.hxx"

TRC_INIT_MODULE(iqrf::IbmMessaging);
...
namespace iqrf {
  ...
  public:
    ...
    void activate(const shape::Properties *props)
    {
      ...
      TRC_INFORMATION(std::endl <<
        "******************************" << std::endl <<
        "IbmMessaging instance activate" << std::endl <<
        "******************************"
      );
  ...
  //////////////////
  // IbmMessaging implementation
  //////////////////

  IbmMessaging::IbmMessaging()
  {
    TRC_FUNCTION_ENTER("");
    m_impl = shape_new IbmMessagingImpl();
    TRC_FUNCTION_LEAVE("")
  }
  ...
  void IbmMessaging::registerMessageHandler(MessageHandlerFunc hndl)
  {
    m_impl->registerMessageHandler(hndl);
  }

  void IbmMessaging::unregisterMessageHandler()
  {
  ...
  bool IbmMessaging::acceptAsyncMsg() const
  {
    return m_impl->acceptAsyncMsg();
  }

  void IbmMessaging::activate(const shape::Properties *props)
  {
    return m_impl->activate(props);
  }

  void IbmMessaging::deactivate()
  {
    return m_impl->deactivate();
  }

  void IbmMessaging::modify(const shape::Properties *props)
  {
    m_impl->modify(props);
  }

  void IbmMessaging::attachInterface(shape::IMqttService* iface)
  {
    m_impl->attachInterface(iface);
  }

  void IbmMessaging::detachInterface(shape::IMqttService* iface)
  {
    m_impl->detachInterface(iface);
  }

  void IbmMessaging::attachInterface(shape::ITraceService* iface)
  {
    shape::Tracer::get().addTracerService(iface);
  }

  void IbmMessaging::detachInterface(shape::ITraceService* iface)
  {
