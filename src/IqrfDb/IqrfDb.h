...
namespace iqrf {
  ...
  public:
     ...
     */
    std::vector<Device> getDevices(const std::vector<uint8_t>& requestedDevices = {}) override;
     ...
     * @return Product
     */
    std::optional<Product> getProduct(const uint32_t productId) override;
     ...
     */
    std::unordered_map<uint32_t, Product> getProductsMap(const std::set<uint32_t>& ids) override;
