...
namespace iqrf {
  ...
  public:
    ...
    void handleMsg(const std::string & messagingId, const IMessagingSplitterService::MsgType & msgType, rapidjson::Document doc)
    {
      ...
      try {
        m_iJsRenderService->callFenced(com->getNadr(), methodRequestName, com->getParamAsString(), rawHdpRequest);
      }
      ...
      if (driverRequestError) {
        ...
        if (res->isResponded()) {
          ...
          if (0 == rcode) {
            ...
            try {
              m_iJsRenderService->callFenced(nadrRes, methodResponseName, rawHdpResponse, rspObjStr);
            }
            catch (std::exception &e) {
