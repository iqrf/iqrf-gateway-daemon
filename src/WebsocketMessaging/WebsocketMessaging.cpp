...
namespace iqrf {
  ...
  void WebsocketMessaging::sendMessage(const MessagingInstance& messaging, const std::basic_string<uint8_t>& msg) {
    ...
    std::string auxMessaging(messaging.instance);
    std::size_t sessionId;
    if (std::string::npos != auxMessaging.find_first_of('/')) {
      std::string buf(auxMessaging);
      std::replace(buf.begin(), buf.end(), '/', ' ');
      std::istringstream is(buf);
      is >> auxMessaging >> sessionId;
    }
    std::string message(msg.begin(), msg.end());

    m_wsServer->send(sessionId, message);
    TRC_FUNCTION_LEAVE("")
  }
  ...
  int WebsocketMessaging::handleMessageFromWebsocket(const std::size_t sessionId, const std::string& msg) {
    ...
    if (m_messageHandlerFunc) {
      ...
      auxMessaging.instance += '/' + std::to_string(sessionId);
