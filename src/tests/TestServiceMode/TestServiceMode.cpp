...
namespace iqrf {
  ...
  protected:
    ...
    void SetUp() override {
      ...
      auto result = test_utils::api_token::remove(db, noservice.getId());
      if (!result.status) {
        FAIL() << "Failed to remove API token: " << *result.error;
      }
      result = test_utils::api_token::remove(db, service.getId());
      if (!result.status) {
        FAIL() << "Failed to remove API token: " << *result.error;
      }
      result = test_utils::api_token::insert(db, noservice, timestamp, expiration);
      if (!result.status) {
        FAIL() << "Failed to insert API token: " << *result.error;
      }
      result = test_utils::api_token::insert(db, service, timestamp, expiration);
      if (!result.status) {
        FAIL() << "Failed to insert API token: " << *result.error;
      }
    }
  ...
  TEST_F(ServiceModeTest, service_activate_deactivate) {
    ...
    buffer.consume(buffer.size());
    doc = test_utils::json::parseJsonString(response);
    ASSERT_FALSE(doc.HasParseError());
    // check response
    EXPECT_STREQ("mngService_Deactivate", Pointer("/mType").Get(doc)->GetString());
    EXPECT_STREQ("d6b05c55-408b-459d-bc25-f74c42fa0153", Pointer("/data/msgId").Get(doc)->GetString());
