...
namespace iqrf {
  ...
  TEST_F(ServiceModeTest, service_legacy_mode_after_activate_fail) {
    auto ws = wsConnectAuth(service_string);
    beast::error_code ec;
    // send activate
    ws.write(net::buffer(ServiceModeTest::ACTIVATE), ec);
  ...
  TEST_F(ServiceModeTest, service_tr_write_not_active) {
    auto ws = wsConnectAuth(service_string);
    beast::error_code ec;
    // send tr write
    ws.write(net::buffer(ServiceModeTest::TRWRITE), ec);
  ...
  TEST_F(ServiceModeTest, service_tr_write_legacy_service_mode) {
    auto ws = wsConnectAuth(service_string);
    beast::error_code ec;
    // send legacy service mode
    ws.write(net::buffer(ServiceModeTest::MODE_SERVICE), ec);
  ...
  TEST_F(ServiceModeTest, service_tr_write_not_service_session_owner) {
    auto ws1 = wsConnectAuth(service_string);
    auto ws2 = wsConnectAuth(service_string);
    beast::error_code ec;
    // send activate from client 1
  ...
  TEST_F(ServiceModeTest, service_tr_write_success) {
    auto ws = wsConnectAuth(service_string);
    beast::error_code ec;
    // send activate
    ws.write(net::buffer(ServiceModeTest::ACTIVATE), ec);
