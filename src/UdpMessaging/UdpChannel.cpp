...
UdpChannel::UdpChannel(unsigned short remotePort, unsigned short localPort, unsigned int expiration, unsigned dataBuffSize) {
  ...
  if (m_sockfd == -1) {
    THROW_EXC_TRC_WAR(UdpChannelException, "socket failed: " << strerror(errno) << '(' << errno << ')');
  }
  ...
  if (ret == -1) {
    close(m_sockfd);
    THROW_EXC_TRC_WAR(UdpChannelException, "setsockopt failed: " << strerror(errno) << '(' << errno << ')');
  }
  ...
  if (ret == -1) {
    close(m_sockfd);
    THROW_EXC_TRC_WAR(UdpChannelException, "bind failed: " << strerror(errno) << '(' << errno << ')');
  }
...
void UdpChannel::sendTo(const std::basic_string<unsigned char>& message) {
  ...
  if (sentBytes == -1) {
    THROW_EXC_TRC_WAR(UdpChannelException, "Failed to send message, sendto: " << strerror(errno) << '(' << errno << ')');
  }
}
...
void UdpChannel::listen() {
  ...
  try {
    ...
    while (m_runListenThread) {
      ...
      if (recBytes == -1) {
        THROW_EXC_TRC_WAR(UdpChannelException, "Failed to receive message, recvmsg: " << strerror(errno) << '(' << errno << ')');
      }
...
void UdpChannel::findInterfaceByIndex(const int &idx) {
  ...
  if (sockfd == -1) {
    TRC_WARNING("Failed to create socket for interface info: " << strerror(errno) << '(' << errno << ')');
    return;
  }

  res = ioctl(sockfd, SIOCGIFCONF, (char *)&ifc);
  if (res == -1) {
    TRC_WARNING("Interface configuration ioctl failed: " << strerror(errno) << '(' << errno << ')');
    close(sockfd);
    return;
  }
  ...
  for (unsigned int i = 0; i < ifc.ifc_len/sizeof(struct ifreq); ++i) {
    ...
    if (res == -1) {
      TRC_WARNING("Interface index ioctl failed: " << strerror(errno) << '(' << errno << ')');
      break;
    }
    ...
    if (res == -1) {
      TRC_WARNING("Interface IP ioctl failed: " << strerror(errno) << '(' << errno << ')');
      break;
    }
    ...
    if (res == -1) {
      TRC_WARNING("Interface MAC ioctl failed: " << strerror(errno) << '(' << errno << ')');
      break;
    }
