...
namespace jutils
{
  ...
  inline void parseJsonFile(const std::string& fname, rapidjson::Document& json)
  {
    ...
    if (json.HasParseError()) {
      THROW_EXC_TRC_WAR(std::logic_error, "Json parse error: " << NAME_PAR(emsg, json.GetParseError()) <<
  ...
  {
    if (!v.IsObject())
      THROW_EXC_TRC_WAR(std::logic_error, "Expected: Json Object, detected: " << PAR(name) << NAME_PAR(type, v.GetType()));
  }
  ...
  {
    if (!v.IsArray())
      THROW_EXC_TRC_WAR(std::logic_error, "Expected: Json Array, detected: " << PAR(name) << NAME_PAR(type, v.GetType()));
  }
  ...
  inline const rapidjson::Value::ConstMemberIterator getMember(const std::string& name, const rapidjson::Value& jsonValue)
  {
    ...
    if (m == jsonValue.MemberEnd()) {
      THROW_EXC_TRC_WAR(std::logic_error, "Expected member: " << PAR(name));
    }
    return m;
  }
