...
namespace iqrf {
  ...
  public:
    Impl(const std::string& path): directory(path) {
  ...
  MigrationManager::MigrationManager(const std::string& path): impl_(std::make_unique<Impl>(path)) {}

  MigrationManager::~MigrationManager() = default;

  std::size_t MigrationManager::migrate(std::shared_ptr<SQLite::Database> connection) {
    return impl_->migrate(connection);
  }
}
