...
namespace iqrf {
	...
	public:
		...
		void deactivate() {
			...
			m_connector->unregisterResponseHandler(std::move(*m_accessToken));
		...
		void modify(const shape::Properties *props) {
			...
			m_uartEnableGpio = gpio::Gpio(gpio::GpioConfig(uartEnablePin));
			m_spiEnableGpio = gpio::Gpio(gpio::GpioConfig(spiEnablePin));
			m_i2cEnableGpio = gpio::Gpio(gpio::GpioConfig(i2cEnablePin));
	...
	private:
		...
		std::unique_ptr<connector::AccessToken> m_accessToken = nullptr;
	};
