...
namespace iqrf
{
	...
	public:
		...
		bool parseFromFile(const std::string &fname, rapidjson::Document &doc)
		{
			...
			{
				TRC_WARNING("Json parse error: " << NAME_PAR(emsg, doc.GetParseError()) << NAME_PAR(eoffset, doc.GetErrorOffset()));
			}
			else
			{
				retval = true;
...
#define POINTER_GET_INT64(iterKey, iter, key, val, fileName)                                               \
	{                                                                                                      \
		...
		void updateCacheStandard()
		{
			...
			for (auto &stdItem : m_standardMap)
			{ // 1
			...
			for (auto &stdItem : m_standardMap)
			{ // 2
				for (auto &stdDriver : stdItem.second.m_drivers)
				{ // 3
	  ...
	  for (auto & pck : m_packageMap) { // 2
		...
		std::tuple<CacheStatus, std::string> invokeWorker() {
			TRC_FUNCTION_ENTER("");
