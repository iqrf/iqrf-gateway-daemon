...
namespace iqrf {
    ...
    if (!cacheExists()) {
      if (m_downloadIfRepoCacheEmpty) {
        ...
        try {
          ...
          TRC_WARNING("[IQRF Repository cache] Failed to download remote cache or validate cache contents: " << e.what());
          std::cerr << "[IQRF Repository cache] Failed to downlaod remote cache or validate cache contents." << e.what() << std::endl;
        }
      } else {
      ...
      if (m_cacheStatus == CacheStatus::UPDATE_NEEDED) {
        ...
        try {
          ...
          TRC_WARNING("[IQRF Repository cache] Failed to download or save cache to filesystem, or validate cache contents: " << e.what());
          std::cerr << "[IQRF Repository cache] Failed to download or save cache to filesystem, or validate cache contents: " << e.what() << std::endl;
        }
  ...
  void JsCache::downloadCache() {
    ...
    // move old cache to cache.old dir
    ...
    std::filesystem::copy(tmpServerStatePath, m_serverStateFilePath, std::filesystem::copy_options::overwrite_existing);
