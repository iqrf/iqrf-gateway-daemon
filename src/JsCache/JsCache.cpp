  ...
  void JsCache::updateCacheProducts() {
    ...
    for (auto itr = doc.begin(); itr != doc.end(); ++itr) {
      ...
      try {
        ...
        std::shared_ptr<metadata::ProductMetadata> metadata = nullptr;

        if (!productDoc["metadata"].empty() && productDoc["metadata"].contains("static")) {
          uint32_t newestSupported = MIN_METADATA_VERSION;
          json metadataDoc = nullptr;

          for (const auto& obj : productDoc["metadata"]["static"]) {
            uint32_t version = obj["version"];
            if (metadataDoc.is_null() || (version > newestSupported && version <= MAX_METADATA_VERSION)) {
              newestSupported = version;
              metadataDoc = obj;
            }
          }

          if (!metadataDoc.is_null()) {
            try {
              metadata = std::make_shared<metadata::ProductMetadata>(
                metadata::ProductMetadataParser::parse(metadataDoc)
              );
            } catch (const std::exception &e) {
              TRC_WARNING(
                "Failed to parse static metadata version " << newestSupported
                << " of product HWPID " << hwpid
                << ": " << e.what()
              );
            }
          }
