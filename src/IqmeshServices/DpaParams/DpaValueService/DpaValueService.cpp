...
namespace iqrf {
	...
	void DpaValueService::handleMsg(const MessagingInstance& messaging, const IMessagingSplitterService::MsgType &msgType, Document doc) {
	...
	void DpaValueService::activate(const shape::Properties *props) {
		...
		m_splitterService->registerFilteredMsgHandler(
			m_mTypes, [&](const MessagingInstance &messaging, const IMessagingSplitterService::MsgType &msgType, Document doc) {
