...
namespace iqrf {
  ...
  public:
    ...
    ReadTrConfigError(const ReadTrConfigError& other) {
  ...
  private:
    ...
    ReadTrConfigError m_enumPerError;
  ...
  public:
    ...
    ReadTrConfigError getEnumPerError() const { return m_enumPerError; };
    void setEnumPerError(const ReadTrConfigError& error)
    {
      m_enumPerError = error;
    }
  ...
  private:
    ...
    void getPerInfo(ReadTrConfigResult& readTrConfigResult, const uint16_t deviceAddress)
    {
      ...
      {
        ReadTrConfigError error(ReadTrConfigError::Type::PerEnumError, e.what());
        readTrConfigResult.setEnumPerError(error);
      }
      readTrConfigResult.addTransactionResult(transResult);
      TRC_FUNCTION_LEAVE("");
    }
    ...
    void handleMsg(const std::string& messagingId, const IMessagingSplitterService::MsgType& msgType, rapidjson::Document doc)
    {
      ...
      {
        // Enumberate peripheral 
        getPerInfo(readTrConfigResult, deviceAddr);
      ...
      {
        // Yes, getPerInfo transactions success ?
        readTrConfigError = readTrConfigResult.getEnumPerError();
        if (readTrConfigError.getType() == ReadTrConfigError::Type::NoError)
        {
          // Yes, readTrConfig transactions success ?
