...
namespace iqrf {
	...
	std::set<uint8_t> RemoveBondService::FRCAcknowledgedBroadcastBits(RemoveBondResult& removeBondResult, const uint8_t PNUM, const uint8_t PCMD, const uint16_t hwpId, const std::set<uint8_t> &nodes) {
		...
		try {
			...
			// Clear UserData
			memset((void*)frcAckBroadcastPacket.DpaRequestPacket_t.DpaMessage.PerFrcSendSelective_Request.UserData, 0, sizeof(frcAckBroadcastPacket.DpaRequestPacket_t.DpaMessage.PerFrcSend_Request.UserData));
			// DPA request
	...
	void RemoveBondService::nodeRemoveBondBatch(RemoveBondResult& removeBondResult, const std::set<uint8_t> &nodes, const uint16_t hwpId) {
		TRC_FUNCTION_ENTER("");
		std::unique_ptr<IDpaTransactionResult2> transResult;
		try {
			auto nodeItr = nodes.begin();
			do {
				...
				DpaMessage removeBondRequest;
				DpaMessage::DpaPacket_t removeBondPacket;
				removeBondPacket.DpaRequestPacket_t.NADR = *nodeItr++;
				removeBondPacket.DpaRequestPacket_t.PNUM = PNUM_OS;
				removeBondPacket.DpaRequestPacket_t.PCMD = CMD_OS_BATCH;
				removeBondPacket.DpaRequestPacket_t.HWPID = hwpId;
	...
	void RemoveBondService::removeBond(RemoveBondResult& removeBondResult) {
		...
		try {
			...
			if (m_requestParams.wholeNetwork || m_requestParams.deviceAddrList.size() > 1) {
				...
				if (coordParams.dpaVerWord < 0x0400) {
					...
					nodeRemoveBondBatch(removeBondResult, std::set<uint8_t>{BROADCAST_ADDRESS}, m_requestParams.hwpId);
					...
					if (m_requestParams.wholeNetwork) {
						nodes = bondedNodes; // SHOULD THIS BE ALL NODES?
						for (auto node : nodes) {
				...
				if (bondedNodes.find(nodeAddr) != bondedNodes.end()) {
					...
					if (coordParams.dpaVerWord < 0x0400) {
						nodeRemoveBondBatch(removeBondResult, m_requestParams.deviceAddrList, m_requestParams.hwpId);
					} else {
	...
	void RemoveBondService::removeBondOnlyInC(RemoveBondResult& removeBondResult) {
		...
		try {
			...
			std::set<uint8_t> bondedNodes = getBondedNodes(removeBondResult);
			// Clear all bonds ?
			if (m_requestParams.wholeNetwork == true) {
				// Yes, clear all bonds
				clearAllBonds(removeBondResult);
			} else {
