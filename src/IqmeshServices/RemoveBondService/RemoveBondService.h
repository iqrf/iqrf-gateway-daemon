...
namespace iqrf {
	...
	private:
		...
		/**
		 * Remove multiple bonds at [N] and restart nodes
		 * @param removeBondResult Service result
		 * @param nodes Node addresses to remove
		 * @param hwpId HWPID filter
		 */
		void nodeRemoveBondBatch(RemoveBondResult& removeBondResult, const std::set<uint8_t> &nodes, const uint16_t hwpId);
		...
		/// Remove bond in coordinator message type
