...
namespace iqrf {
	...
	typedef struct {
		/// @brief List of device addresses to remove
		std::set<uint8_t> deviceAddrList;
		/// @brief Unbond all devices from network
		bool wholeNetwork = false;
		/// @brief Hardware profile ID filter
		uint16_t hwpId = HWPID_DoNotCheck;
		/// @brief Number of transaction repeats on failure
		int repeat = 1;
	...
	private:
		...
		void parse(rapidjson::Document& doc) {
			...
			if ((jsonValue = rapidjson::Pointer("/data/req/wholeNetwork").Get(doc))) {
				m_requestParams.wholeNetwork = jsonValue->GetBool();
			}
			// hwpId
